...
void OverlayFacemask(Mat& frame, String path)
{
	bool stop = false;
	//训练好的文件名称
	String faceCascadeName = path + "/cascadeFiles/haarcascade_frontalface_alt.xml";
	String maskFileName = path + "/shrek_mask.png";
	...
	Mat faceMask = imread(maskFileName);
    ...
    vector<Rect> faces;
        ...
        for(int i = 0; i < faces.size(); i++)
        {
            ...
            int x = faces[i].x - int(0.05*faces[i].width);
            int y = faces[i].y - int(0.1*faces[i].height);
            int w = int(1.1 * faces[i].width);
            int h = int(1.1 * faces[i].height);
